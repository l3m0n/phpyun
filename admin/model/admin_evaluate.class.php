...
class admin_evaluate_controller extends common {
	...
	function message_action(){
		...
		$urlarr['page']="{{page}}";
		$pageurl=Url($_GET['m'],$urlarr,'admin'); 
		$rows=$this->get_page("evaluate_leave_message","1 order by `id` desc",$pageurl,$this->config['sy_listnum']);
		if($rows&&is_array($rows)){
			...
			$friend=$this->obj->DB_select_all("friend_info","`uid` in(".pylode(',',$uid).")","`uid`,`nickname`");
			foreach($rows as $key=>$val){
				foreach($friend as $v){
					if($v['uid']==$val['uid']){
