...
class friend_controller extends ask_controller{
	...
	function myatn_action(){
		...
		$pageurl=Url('ask',array("c"=>$_GET['c'],'a'=>'myatn','uid'=>$uid,"page"=>"{{page}}"));
	...
	function save_action(){
		if($_POST['submit']){
			...
			if($nid){ 
				$result=$this->max_time('朋友圈留言');				
				if($result==true){  
					$this->company_invtal($this->uid,$this->config['integral_friend_msg'],$auto,"朋友圈留言",true,2,'integral'); 
				}
