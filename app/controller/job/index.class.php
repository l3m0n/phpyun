...
class index_controller extends job_controller{
	function comsearch(){
		...
		$FinderParams=array('keyword','hy','job','city','job1','job1_son','job_post','provinceid','cityid','three_cityid','salary','edu','exp','sex','type','report','sdate','uptime');
		foreach($_GET as $k=>$v){
	...
	function report_action(){
		...
		if(md5($_POST['authcode'])!=$_SESSION['authcode'] || empty($_SESSION['authcode'])){unset($_SESSION['authcode']);echo 1;die;}
		$row=$AskM->GetReportOne(array('p_uid'=>$this->uid,'c_uid'=>(int)$_POST['r_uid'],'usertype'=>$this->usertype));
	...
	function recommend_action(){
		if($_POST){
			...
			$sendid = $smtp->sendmail($this->config['sy_smtpnickname'],$_POST['femail'],$smtpusermail,$title,$contents,"HTML","","","",$myemail);
			if($sendid){
				echo 1;
			}else{
