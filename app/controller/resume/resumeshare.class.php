...
class resumeshare_controller extends resume_controller{
	function index_action(){
		if($_POST){
			...
			$sendid = $smtp->sendmail($this->config['sy_smtpnickname'],$_POST['femail'],$smtpusermail,"您的好友".$myemail."向您推荐了简历！",$contents,"HTML","","","",$myemail);
			if($sendid){
			    $this->MODEL()->insert_into("email_msg",array('uid'=>'','name'=>$_POST['femail'],'cuid'=>$this->uid,'cname'=>$myemail,'email'=>$_POST['femail'],'title'=>'简历分享','content'=>'','state'=>'1','ctime'=>time()));
				echo 1;
			}else{
			    $this->MODEL()->insert_into("email_msg",array('uid'=>'','name'=>$_POST['femail'],'cuid'=>$this->uid,'cname'=>$myemail,'email'=>$_POST['femail'],'title'=>'简历分享','content'=>'','state'=>'0','ctime'=>time()));
