...
class index_controller extends common
{
	...
	function index_action()
	{
		if($this->config['did']){
			...
			header('Location: '.$this->config['sy_indexdomain'].'/index.php?m=wxconnect');
			die;
		}
		...
		if($_GET['state'] == $_SESSION['wx']['state'])
		{
			...
			$str=curl_exec ($ch);
			curl_close ($ch);
			$str = str_replace("%E2%80%86","",urlencode($str)); 
			$str = urldecode($str);
	...
	function wxbind_action()
	{
		...
		if(((int)$_GET['usertype']=='1' || (int)$_GET['usertype']=='2') && $_SESSION['wx']['openid'])
		{
				...
				$userid=$this->obj->DB_insert_once("member","`username`='$username',`password`='$pass',`usertype`='$usertype',`status`='1',`salt`='$salt',`reg_date`='$time',`reg_ip`='$ip',`wxopenid`='".$_SESSION['wx']['openid']."',`unionid`='".$_SESSION['wx']['unionid']."',`login_date`='".time()."',`login_ip`='".$ip."'");
 				if(!$userid)
				{
