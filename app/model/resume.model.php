	...
	function resume_select($id,$user_jy=''){
		...
		if($user_jy['uid']){
			...
			if($user['uid']){
				...
				if($this->config['user_name']==3 && $this->uid!=$user_jy['uid']){
					$user["username_n"] = "NO.".$user_jy['id'];
				}elseif($this->config['user_name']==2 && $this->uid!=$user_jy['uid']){
				...
				if($this->config['sy_usertype_1']=='1'&&$this->uid!=$user['uid']){
					if($this->usertype=='2'){
						$thisy_down=$this->DB_select_all("down_resume","`comid`='".$this->uid."'","uid");
					}
					if(!empty($thisy_down)){
