...
function checkform(){
	...
	if($("#askcheckcode").val()=='1'){
		noplaceholder('ask_CheckCode');
		var authcode=$("#ask_CheckCode").val();
		if(authcode==''){
			layer.msg('请填写验证码！', 2, 8);return false;
		} 
	}var content=$("#content").val();
	$.post(weburl+"/ask/index.php?c=savecode",{authcode:authcode},function(data){
		if(data==1){
			layer.msg('验证码错误！', 2, 8,function(){check_codes();});return false;
		}
