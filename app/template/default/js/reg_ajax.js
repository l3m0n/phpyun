...
function sendmsg(){
	...
	if(date==1 && send==0){
		...
		$.post(weburl+"/index.php?m=ajax&c=regcode",{moblie:moblie,code:code},function(data){ 
			layer.closeAll();
			if(data==0){
				layer.msg('手机不能为空！', 2, 8,function(){check_code();});return false; 
			}else if(data==1){
				layer.msg('同一手机号一天发送次数已超！', 2, 8,function(){check_code();});
			}else if(data==2){
				layer.msg('同一IP一天发送次数已超！', 2, 8,function(){check_code();});
			}else if(data==3){
				layer.msg('短信还没有配置，请联系管理员！', 2, 8,function(){check_code();});return false; 
			}else if(data==4){
				layer.msg('请不要频繁重复发送！', 2, 8,function(){check_code();});return false; 
			}else if(data==5){
				layer.msg('图片验证码错误！', 2, 8,function(){check_code();});return false; 
			}else if(data=="发送成功!"){
				sendtime("121"); 
			}else{
				layer.msg(data, 2, 8,function(){check_code();});return false; 
			}
		})
	}
}
