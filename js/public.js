...
function checkcodes(){
	document.getElementById("vcodeimgs").src=weburl+"/app/include/authcode.inc.php?"+Math.random();
}
...
function reportSub(){
	...
	$.post(weburl+"/job/index.php?c=report",{authcode:authcode,r_reason:r_reason,id:id,r_name:r_name,r_uid:r_uid},function(data){
		if(data==1){
			layer.msg('验证码不正确！', 2, 8,function(){checkauthcode();});
		}else if(data==2){
			layer.msg('您已经举报过该用户！', 2, 8,function(){checkauthcode();});
		}else if(data==3){
			layer.closeAll();
			layer.msg('举报成功！', 2,9);
		}else if(data==4){
			layer.msg('举报失败！', 2, 8,function(){checkauthcode();});
		}else if(data==5){
			layer.msg('网站已关闭举报功能！', 2, 8,function(){checkauthcode();});
		}
	})
}
