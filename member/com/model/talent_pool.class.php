...
class talent_pool_controller extends company
{
	function index_action()
	{
		...
		{
			$resume=$this->obj->DB_select_alls("resume","resume_expect","a.uid=b.uid and a.`r_status`<>'2' and a.`name` like '%".trim($_GET['keyword'])."%'","a.`name`,a.`uid`,a.`sex`,a.`edu`,a.`exp`,b.`salary`,b.`job_classid`");
			if(is_array($resume))
			{
				foreach($resume as $v)
				{
